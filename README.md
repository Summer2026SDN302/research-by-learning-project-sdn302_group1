...
- **AI & LLM Service (Google Gemini 1.5 Flash API):** Thay vì tự huấn luyện (train) một mô hình AI từ đầu với chi phí khổng lồ và dữ liệu phức tạp, nhóm quyết định tích hợp API của LLM bên thứ ba. Kết hợp cùng kỹ thuật Prompt Engineering trực tiếp trên Backend NodeJS, hệ thống có thể tạo ra một "nhân cách" AI chuyên hỗ trợ tâm lý với độ thấu cảm cao và xử lý tiếng Việt mượt mà.
---
